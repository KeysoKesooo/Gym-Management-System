...
import useStaff from "@/hooks/useMemberManagement";
...
function AdminContent({ user }: { user: any }) {
  ...
  const { member, loading, error, addMember, updateMember, deleteMember } = useStaff(token);
                ...
                <tr key={m.id}>
                  ...
                  <td className="p-2 border">
                    <button
                      ...
                      onClick={() => {
                        setEditing(m);
                        setUpdateData({ name: m.name, email: m.email, role: m.role });
                      }}
                    >
                      Edit
                    </button>
                    <button
                      ...
                      onClick={() => m.id && deleteMember(m.id)}
                    >
                      Delete
                    </button>
                  </td>
          ...
          <div style={modalStyle}>
            ...
            <div className="flex gap-2 mt-2">
              <button
                className="px-3 py-1 bg-green-600 text-white rounded"
                onClick={() => {
                  if (editing.id) updateMember(editing.id, updateData);
                  setEditing(null);
                }}
              >
                Update
